...
async function findGameByCode(code) {
  ensureSupabase();
  const { data, error } = await supabaseClient.from("games").select("*").eq("code", normalizeLobbyCode(code)).single();
...
async function joinGame(game, guestName) {
  ...
  const { data: player, error } = await supabaseClient.from("game_players").insert({
    game_id: game.id,
    user_id: null,
    guest_name: cleanPlayerName(guestName),
  ...
  await supabaseClient.from("games").update({ status: "playing", turn_started_at: new Date().toISOString() }).eq("id", game.id);
  return player;
}
...
async function saveMove({ gameId, gamePlayerId, guestName, animalName, requiredLetter, moveNumber }) {
  ...
  const { data, error } = await supabaseClient.from("moves").insert({
    game_id: gameId,
...
async function eliminatePlayer(playerId) {
  const { error } = await supabaseClient.from("game_players").update({
    is_eliminated: true,
...
function initPracticePage() {
  ...
  const el = mapElements({
    lastAnimal: "#practiceLastAnimal", requiredLetter: "#practiceRequiredLetter", moveCount: "#practiceMoveCount",
    animalForm: "#practiceAnimalForm", animalInput: "#practiceAnimalInput", message: "#practiceMessage",
    hintButton: "#practiceHintButton", newRoundButton: "#practiceNewRoundButton", movesList: "#practiceMovesList",
    suggestForm: "#practiceSuggestForm", suggestInput: "#practiceSuggestInput", animalCount: "#practiceAnimalCount"
  });
  ...
  el.suggestForm.addEventListener("submit", handleSuggest);
  start();

  async function start() {
    try {
      state.animals = await loadApprovedAnimals();
      if (!state.animals.length) return setMessage(el.message, "Keine Tiere in Supabase gefunden.", "warning");
      startNewRound(false);
  ...
  async function handleMove(event) {
    ...
    if (!validation.ok) return setMessage(el.message, validation.message, validation.type);
  ...
  function computerMove() {
    const options = availableAnimals(state.animals, state.requiredLetter, state.moves.map((move) => move.animal));
    if (!options.length) return setMessage(el.message, `Der Computer findet kein Tier mit ${state.requiredLetter.toUpperCase()}. Du gewinnst!`, "success");
  ...
  function validateAnimal(animalName) {
    ...
    if (getFirstLetter(normalized) !== state.requiredLetter) return { ok: false, type: "error", message: `Dein Tier muss mit ${state.requiredLetter.toUpperCase()} anfangen.` };
    if (!findAnimal(state.animals, animalName)) return { ok: false, type: "error", message: `"${animalName}" ist nicht in der Supabase-Tierdatenbank.` };
    if (state.moves.some((move) => normalizeAnimalName(move.animal) === normalized)) return { ok: false, type: "error", message: `"${animalName}" wurde schon gespielt.` };
    return { ok: true };
  }
  ...
  function showHint() {
    ...
    if (!options.length) return setMessage(el.message, `Kein Tipp für ${state.requiredLetter.toUpperCase()} gefunden.`, "warning");
  ...
  async function handleSuggest(event) {
    event.preventDefault();
    try {
      await suggestAnimal(el.suggestInput.value);
      el.suggestInput.value = "";
      setMessage(el.message, "Tier wurde vorgeschlagen.", "success");
    } catch (error) { setMessage(el.message, error.message, "error"); }
  }
...
function initOnlinePage() {
  const state = { animals: [], guestName: "Gast", game: null, localPlayer: null, players: [], moves: [], refreshTimer: null, countdownTimer: null };
  const el = mapElements({
    nameForm: "#onlineNameForm", guestName: "#onlineGuestName", timerEnabled: "#onlineTimerEnabled", turnSeconds: "#onlineTurnSeconds",
    createLobbyButton: "#onlineCreateLobbyButton", lobbyTicket: "#onlineLobbyTicket", lobbyCode: "#onlineLobbyCode", copyCodeButton: "#onlineCopyCodeButton",
    joinForm: "#onlineJoinForm", joinCode: "#onlineJoinCode", message: "#onlineMessage", lastAnimal: "#onlineLastAnimal", requiredLetter: "#onlineRequiredLetter",
    timerDisplay: "#onlineTimerDisplay", turnBadge: "#onlineTurnBadge", moveForm: "#onlineMoveForm", animalInput: "#onlineAnimalInput",
    refreshButton: "#onlineRefreshButton", newRoundButton: "#onlineNewRoundButton", playersList: "#onlinePlayersList", movesList: "#onlineMovesList",
    suggestForm: "#onlineSuggestForm", suggestInput: "#onlineSuggestInput", animalCount: "#onlineAnimalCount"
  });
  ...
  async function copyLobbyCode() {
    ...
    try { await navigator.clipboard.writeText(state.game.code); setMessage(el.message, "Lobby-Code kopiert.", "success"); }
    catch { setMessage(el.message, "Kopieren ging nicht. Markiere den Code manuell.", "warning"); }
  }
  ...
  async function refreshLobby() {
    ...
    try {
      ...
      if (state.game.timer_enabled) await checkTimerExpiration();
      render();
    } catch (error) { setMessage(el.message, error.message, "error"); }
  }
  ...
  async function checkTimerExpiration() {
    ...
    if (!currentPlayer || active.length <= 1 || !state.game.turn_started_at || getRemainingSeconds() > 0) return;
    await eliminatePlayer(currentPlayer.id);
    const updated = state.players.map((p) => p.id === currentPlayer.id ? { ...p, is_eliminated: true } : p);
    const nextTurnOrder = getNextActiveTurnOrder(updated, currentPlayer.turn_order);
    const remaining = updated.filter((p) => !p.is_eliminated);
    await updateGameTurn(state.game.id, nextTurnOrder || currentPlayer.turn_order, remaining.length <= 1 ? "finished" : "playing");
  ...
  async function handleMove(event) {
    event.preventDefault();
    if (!state.game || !state.localPlayer) return setMessage(el.message, "Du bist in keiner Lobby.", "warning");
    const currentPlayer = getCurrentOnlinePlayer();
    if (!currentPlayer || currentPlayer.id !== state.localPlayer.id) return setMessage(el.message, "Du bist gerade nicht dran.", "warning");
    if (currentPlayer.is_eliminated) return setMessage(el.message, "Du bist ausgeschieden und kannst nur noch zuschauen.", "warning");
    const animalName = cleanAnimalName(el.animalInput.value);
    const validation = validateOnlineAnimal(animalName);
    if (!validation.ok) return setMessage(el.message, validation.message, validation.type);
    try {
      const nextRequiredLetter = getLastLetter(animalName);
      await saveMove({ gameId: state.game.id, gamePlayerId: state.localPlayer.id, guestName: state.guestName, animalName, requiredLetter: state.game.current_required_letter, moveNumber: state.moves.length + 1 });
  ...
  function validateOnlineAnimal(animalName) {
    ...
    if (!animalName) return { ok: false, type: "warning", message: "Bitte gib ein Tier ein." };
    if (getFirstLetter(normalized) !== needed) return { ok: false, type: "error", message: `Das Tier muss mit ${needed.toUpperCase()} anfangen.` };
    if (!findAnimal(state.animals, animalName)) return { ok: false, type: "error", message: `"${animalName}" ist nicht in der Tierdatenbank.` };
    if (state.moves.some((move) => move.normalized_animal_name === normalized)) return { ok: false, type: "error", message: `"${animalName}" wurde schon gespielt.` };
    return { ok: true };
  }
  ...
  async function handleSuggest(event) {
    event.preventDefault();
    try { await suggestAnimal(el.suggestInput.value); el.suggestInput.value = ""; setMessage(el.message, "Tier wurde vorgeschlagen.", "success"); }
    catch (error) { setMessage(el.message, error.message, "error"); }
  }
  ...
  function getRemainingSeconds() {
    if (!state.game?.timer_enabled || !state.game?.turn_started_at) return null;
  ...
  function renderTimerOnly() {
    if (state.game) el.timerDisplay.textContent = formatTimer(getRemainingSeconds());
  }
  ...
  function render() {
    ...
    el.requiredLetter.textContent = state.game?.current_required_letter ? state.game.current_required_letter.toUpperCase() : "---";
    el.timerDisplay.textContent = state.game?.timer_enabled ? formatTimer(getRemainingSeconds()) : "Aus";
    el.turnBadge.textContent = currentPlayer ? `${currentPlayer.guest_name} ist dran${currentPlayer.is_eliminated ? " · ausgeschieden" : ""}` : "Keine aktive Lobby";
    el.animalCount.textContent = `${state.animals.length} Tiere aus Supabase geladen`;
    el.playersList.innerHTML = state.players.length ? state.players.map((p) => `<article class="player-row ${p.is_eliminated ? "eliminated" : ""}"><div><strong>${escapeHtml(p.guest_name)}</strong><div class="meta">Spieler ${p.turn_order}${p.id === state.localPlayer?.id ? " · Du" : ""}</div></div>${p.is_eliminated ? `<span class="pill danger">Raus</span>` : p.turn_order === state.game?.current_turn_order ? `<span class="pill success">Dran</span>` : `<span class="pill">Aktiv</span>`}</article>`).join("") : `<p class="hint">Noch keine Spieler.</p>`;
    el.movesList.innerHTML = state.moves.length ? state.moves.map((m) => `<li><strong>${escapeHtml(m.animal_name)}</strong> <span class="hint">von ${escapeHtml(m.guest_name || "Gast")}</span></li>`).join("") : `<li><strong>${escapeHtml(state.game?.last_animal || "Turmfalke")}</strong> <span class="hint">Starttier</span></li>`;
    if (active.length === 1 && state.players.length > 1) setMessage(el.message, `${active[0].guest_name} gewinnt!`, "success");
  }
}

function initLocalPage() {
  const state = { animals: [], players: [], moves: [], lastAnimal: "Turmfalke", requiredLetter: "e", turnIndex: 0, started: false };
  const el = mapElements({ playerForm: "#localPlayerForm", playerName: "#localPlayerName", startButton: "#localStartButton", playersList: "#localPlayersList", message: "#localMessage", lastAnimal: "#localLastAnimal", requiredLetter: "#localRequiredLetter", moveCount: "#localMoveCount", turnBadge: "#localTurnBadge", moveForm: "#localMoveForm", animalInput: "#localAnimalInput", movesList: "#localMovesList", suggestForm: "#localSuggestForm", suggestInput: "#localSuggestInput", animalCount: "#localAnimalCount" });
  ...
  async function start() {
    try { state.animals = await loadApprovedAnimals(); setMessage(el.message, "Tierdatenbank geladen. Füge Spieler hinzu.", "success"); }
    catch (error) { setMessage(el.message, error.message, "error"); }
    render();
  }
  ...
  function addPlayer(event) {
    ...
    if (!name) return setMessage(el.message, "Bitte gib einen Spielernamen ein.", "warning");
  ...
  function startRound() {
    if (state.players.length < 2) return setMessage(el.message, "Du brauchst mindestens 2 Spieler.", "warning");
    const animal = randomItem(state.animals) || { name: "Turmfalke" };
    state.lastAnimal = animal.name;
    state.requiredLetter = getLastLetter(animal.name);
    state.moves = [];
  ...
  function handleMove(event) {
    event.preventDefault();
    if (!state.started) return setMessage(el.message, "Starte zuerst eine Runde.", "warning");
    const animalName = cleanAnimalName(el.animalInput.value);
    const validation = validateAnimal(animalName);
    if (!validation.ok) return setMessage(el.message, validation.message, validation.type);
  ...
  function validateAnimal(animalName) {
    const normalized = normalizeAnimalName(animalName);
    if (!animalName) return { ok: false, type: "warning", message: "Bitte gib ein Tier ein." };
    if (getFirstLetter(normalized) !== state.requiredLetter) return { ok: false, type: "error", message: `Das Tier muss mit ${state.requiredLetter.toUpperCase()} anfangen.` };
    if (!findAnimal(state.animals, animalName)) return { ok: false, type: "error", message: `"${animalName}" ist nicht in der Tierdatenbank.` };
    if (state.moves.some((move) => normalizeAnimalName(move.animal) === normalized)) return { ok: false, type: "error", message: `"${animalName}" wurde schon gespielt.` };
    return { ok: true };
  }
  ...
  function render() {
    el.playersList.innerHTML = state.players.length ? state.players.map((p, i) => `<article class="player-row"><div><strong>${escapeHtml(p.name)}</strong><div class="meta">Spieler ${i + 1}</div></div>${state.started && i === state.turnIndex ? `<span class="pill success">Dran</span>` : `<span class="pill">Dabei</span>`}</article>`).join("") : `<p class="hint">Noch keine Spieler.</p>`;
    el.lastAnimal.textContent = state.lastAnimal || "---";
    el.requiredLetter.textContent = state.requiredLetter ? state.requiredLetter.toUpperCase() : "---";
    el.moveCount.textContent = String(state.moves.length);
    el.turnBadge.textContent = state.started ? `${state.players[state.turnIndex].name} ist dran` : "Noch keine Runde";
    el.animalCount.textContent = `${state.animals.length} Tiere aus Supabase geladen`;
    el.movesList.innerHTML = state.moves.length ? state.moves.map((m) => `<li><strong>${escapeHtml(m.animal)}</strong> <span class="hint">von ${escapeHtml(m.playerName)}</span></li>`).join("") : `<li><strong>${escapeHtml(state.lastAnimal)}</strong> <span class="hint">Starttier</span></li>`;
  }
}

function ensureSupabase() {
  if (!supabaseClient) throw new Error("Supabase konnte nicht geladen werden.");
  if (!ANIMALCHAIN_CONFIG.supabaseKey || ANIMALCHAIN_CONFIG.supabaseKey.includes("DEIN_PUBLIC")) throw new Error("Bitte trage deinen Supabase Public/Publishable Key in js/app.js ein.");
}
...
function availableAnimals(animals, firstLetter, usedNames = []) {
  ...
  return animals.filter((animal) => animal.first_letter === letter && !used.has(animal.normalized_name));
}

function getNextActiveTurnOrder(players, currentTurnOrder) {
  const active = players.filter((p) => !p.is_eliminated).sort((a, b) => a.turn_order - b.turn_order);
  if (!active.length) return null;
  return (active.find((p) => p.turn_order > currentTurnOrder) || active[0]).turn_order;
}

function firstActiveTurnOrder(players) {
  const active = players.filter((p) => !p.is_eliminated).sort((a, b) => a.turn_order - b.turn_order);
  return active[0]?.turn_order || null;
}
...
function normalizeAnimalName(value) { return String(value || "").trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/ß/g, "ss").replace(/[^a-z\s-]/g, "").replace(/\s+/g, " "); }
function getFirstLetter(value) { return normalizeAnimalName(value).replace(/[^a-z]/g, "").charAt(0) || ""; }
function getLastLetter(value) { const letters = normalizeAnimalName(value).replace(/[^a-z]/g, ""); return letters.charAt(letters.length - 1) || ""; }
function normalizeLobbyCode(value) { return String(value || "").trim().toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 8); }
function generateLobbyCode() { const words = ["WOLF", "FALK", "PANDA", "LUCHS", "ZEBRA", "BIBER", "ADLER", "TIGER"]; return `${randomItem(words)}${Math.floor(100 + Math.random() * 900)}`.slice(0, 8); }
function formatTimer(seconds) { if (seconds === null || seconds === undefined) return "Aus"; const safe = Math.max(0, Number(seconds) || 0); return `${Math.floor(safe / 60)}:${String(safe % 60).padStart(2, "0")}`; }
function toTitleCase(value) { return String(value || "").trim().toLowerCase().split(" ").map((part) => part ? part.charAt(0).toUpperCase() + part.slice(1) : "").join(" "); }
