...
create or replace function public.only_letters(value text)
returns text
language sql
stable
as $$
  select regexp_replace(public.normalize_animal_name(value), '[^a-z]', '', 'g');
$$;

create or replace function public.first_animal_letter(value text)
returns text
language sql
stable
as $$
  select left(public.only_letters(value), 1);
$$;

create or replace function public.last_animal_letter(value text)
returns text
language sql
stable
as $$
  select right(public.only_letters(value), 1);
$$;
...
create table if not exists public.animal_suggestions (
  ...
  created_at timestamptz not null default now(),
  constraint animal_suggestions_name_not_empty check (length(trim(name)) > 0),
